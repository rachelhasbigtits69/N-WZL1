...
async def get_plugins_menu(user_id: int, stype: str = "main"):
 ...
 • <b>Total Loaded:</b> {len(loaded_plugins)}"""
        ...
        for plugin in available_plugins:
            if plugin not in loaded_plugins:
                buttons.data_button(f"[Load] {plugin}", f"plugins {user_id} load {plugin}")
 ...
 • <b>Unloaded Plugins:</b> {unloaded_count}"""
        ...
        for plugin in loaded_plugins:
            status = "[+]" if plugin.enabled else "[ ]"
            enabled_text = "Enabled" if plugin.enabled else "Disabled"
            text += f" • <b>{plugin.name}</b> v{plugin.version}\n"
            text += f" • Author: {plugin.author}\n"
            text += f" • Status: {status} {enabled_text}\n"
        ...
        if plugin_info:
            ...
            enabled_text = "Enabled" if plugin_info.enabled else "Disabled"
            buttons.data_button(
 ...
 • <b>Status:</b> {status} {enabled_text}
...
def register_plugin_commands():
    from bot.core.handlers import TgClient
