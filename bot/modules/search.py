...
async def search(key, site, message, method):
    if method.startswith("api"):
        _q = quote(str(key or ""), safe="")
        _site = quote(str(site or ""), safe="")
        if method == "apisearch":
            LOGGER.info(f"API Searching: {key} from {site}")
            if site == "all":
                api = f"{Config.SEARCH_API_LINK}/api/v1/all/search?query={_q}&limit={Config.SEARCH_LIMIT}"
            else:
                api = f"{Config.SEARCH_API_LINK}/api/v1/search?site={_site}&query={_q}&limit={Config.SEARCH_LIMIT}"
