...
from bot.helper.ext_utils.links_utils import decode_slink
...
async def login(_, message):
    if Config.LOGIN_PASS is None:
        ...
        if sender is None:
            return await send_message(
                message,
                "<b>Login unavailable for anonymous senders.</b>",
            )
        user_id = sender.id
...
async def log(_, message):
    sender = message.from_user or getattr(message, "sender_chat", None)
    if sender is None:
        await send_message(message, "Log fetch unavailable for anonymous senders.")
        return
    uid = sender.id
    buttons = ButtonMaker()
