...
async def list_buttons(user_id, is_recursive=True, user_token=False):
    buttons = ButtonMaker()
    buttons.data_button(
        f"[{'ON' if user_token else 'OFF'}] User Token",
        f"list_types {user_id} ut {is_recursive} {user_token}",
        "header",
    )
    buttons.data_button(
        f"[{'ON' if is_recursive else 'OFF'}] Recursive",
...
async def select_type(_, query):
    ...
    if user_id != int(data[1]):
        ...
        await query.answer()
        is_recursive = not _to_bool(data[3])
        buttons = await list_buttons(user_id, is_recursive, _to_bool(data[4]))
        return await edit_message(message, "Choose list options:", buttons)
    elif data[2] == "ut":
        await query.answer()
        user_token = not _to_bool(data[4])
        buttons = await list_buttons(user_id, _to_bool(data[3]), user_token)
        return await edit_message(message, "Choose list options:", buttons)
    elif data[2] == "cancel":
        await query.answer()
    ...
    await query.answer()
    item_type = data[2]
    is_recursive = _to_bool(data[3])
    user_token = _to_bool(data[4])
