...
from bot import task_dict_lock, task_dict, LOGGER, intervals
...
async def _on_download_started(api, data):
    ...
    try:
        ...
        LOGGER.error(f"onDownloadStarted: failed to fetch state for {gid}: {e}")
        return
...
async def _on_download_error(api, data):
    ...
    error = "aria2 reported a download error (no detail available)."
    with suppress(TimeoutError, ClientError, Exception):
        download, options = await api.tellStatus(gid), await api.getOption(gid)
        raw = download.get("errorMessage", "") or ""
        if raw:
            error = raw
