...
from bot import LOGGER, intervals, jd_listener_lock, jd_downloads
    ...
    while True:
        ...
        async with jd_listener_lock:
            ...
            try:
                ...
                LOGGER.warning(f"JDownloader query_packages failed: {e}")
                continue
            ...
            for d_gid, d_dict in list(jd_downloads.items()):
                if d_dict["status"] == "down":
                    jd_downloads[d_gid]["ids"] = [
                        pid for pid in d_dict["ids"] if pid in all_packages
                    ]
