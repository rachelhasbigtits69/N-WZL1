    ...
    try:
        if photo:
            try:
                ...
                if _recursion_depth >= 3:
                    LOGGER.error("FloodWait: Maximum retry depth reached")
                    return f"FloodWait after 3 retries: {f}"
                await sleep(f.value * 1.2)
                return await send_message(message, text, buttons, block, photo, _recursion_depth + 1)
            except MediaCaptionTooLong:
                if _recursion_depth >= 3:
                    LOGGER.error("MediaCaptionTooLong: Maximum retry depth reached")
                    return
        ...
        if isinstance(message, int):
            try:
                ...
                if message == Config.LINKS_LOG_ID:
                    LOGGER.error(f"LINKS_LOG_ID chat unknown: {message}")
                elif message == Config.MIRROR_LOG_ID:
                    LOGGER.error(f"MIRROR_LOG_ID chat unknown: {message}")
                else:
                    LOGGER.error(f"PeerIdInvalid: {e} - Chat ID: {message}")
                return str(e)
        ...
        if _recursion_depth >= 3:
            ...
            return
        return await send_message(message, text, None, _recursion_depth=_recursion_depth + 1)
    except (MessageEmpty, EntityBoundsInvalid):
        LOGGER.warning(f"MessageEmpty error - text length: {len(text)}, returning silently")
        return None
...
async def edit_message(message, text, buttons=None, block=True, _recursion_depth=0):
    try:
        ...
        if _recursion_depth >= 3:
            LOGGER.error("ReplyMarkupInvalid: Maximum retry depth reached")
            return
        return await edit_message(message, text, None, _recursion_depth=_recursion_depth + 1)
    except FloodWait as f:
        LOGGER.warning(str(f))
        if not block:
            return str(f)
        if _recursion_depth >= 3:
            ...
            return str(f)
        await sleep(f.value * 1.2)
        return await edit_message(message, text, buttons, _recursion_depth=_recursion_depth + 1)
    except Exception as e:
        LOGGER.error(str(e), exc_info=True)
        return str(e)


async def edit_reply_markup(message, buttons, _recursion_depth=0):
    try:
        ...
        LOGGER.warning(str(f))
        if _recursion_depth >= 3:
            LOGGER.error("edit_reply_markup: FloodWait maximum retry depth reached")
            return str(f)
        await sleep(f.value * 1.2)
        return await edit_reply_markup(message, buttons, _recursion_depth + 1)
    except Exception as e:
        LOGGER.error(str(e), exc_info=True)
        return str(e)


async def send_file(message, file, caption="", buttons=None, _recursion_depth=0):
    try:
        ...
        LOGGER.warning(str(f))
        if _recursion_depth >= 3:
            LOGGER.error("send_file: FloodWait maximum retry depth reached")
            return str(f)
        await sleep(f.value * 1.2)
        return await send_file(message, file, caption, buttons, _recursion_depth + 1)
    except Exception as e:
        LOGGER.error(str(e), exc_info=True)
        return str(e)


async def send_rss(text, chat_id, thread_id, _recursion_depth=0):
    try:
        ...
        LOGGER.warning(str(f))
        if _recursion_depth >= 3:
            LOGGER.error("send_rss: FloodWait maximum retry depth reached")
            return str(f)
        await sleep(f.value * 1.2)
        return await send_rss(text, chat_id, thread_id, _recursion_depth + 1)
    except Exception as e:
        LOGGER.error(str(e), exc_info=True)
        return str(e)
...
async def get_tg_link_message(link):
    ...
    if msg is None:
        raise TgLinkException(f"Could not parse Telegram link: {link!r}")
    try:
        chat = msg[2]
        msg_id = msg[3]
    except (IndexError, TypeError) as e:
        raise TgLinkException(
            f"Malformed Telegram link (missing chat/msg id): {link!r}"
        ) from e
    if "-" in msg_id:
...
async def open_dump_btns(message):
    ...
    sender = message.from_user or getattr(message, "sender_chat", None)
    if sender is None:
        return [], False
    user_id = sender.id
    msg_id = message.id
    ...
    udmps = await fetch_user_dumps(user_id)
    if len(udmps) <= 1:
        if udmps:
            return [next(iter(udmps.values()))], False
        return [], False
    ...
    prompt = await send_message(
        message,
        "<b>Select up to 3 dump categories (click to toggle)</b>\n\n"
        "<i>Selected (0/3):</i>\nNone\n\n"
        "<i>Timeout will use ALL dumps if nothing selected.</i>",
        buttons.build_menu(3),
    )

    start_time = time()
    bot_cache[msg_id] = [[], False, False, start_time]

    try:
        while time() - start_time <= 60:
            await sleep(0.5)
            entry = bot_cache.get(msg_id)
            # Cache entry may have been deleted by concurrent cleanup
            if entry is None:
                return [], True
            if entry[1] or entry[2]:
                break

        entry = bot_cache.get(msg_id, [[], False, True, start_time])
        dump_chat, _, is_cancelled, __ = entry
        if not is_cancelled:
            await delete_message(prompt)
            if not dump_chat:
                dump_chat = list(udmps.values())
        else:
