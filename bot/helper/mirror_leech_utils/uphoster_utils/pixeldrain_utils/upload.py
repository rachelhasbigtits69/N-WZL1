    ...
    async def upload(self):
        try:
            ...
            if any(x in err_str.lower() for x in [
                ...
                friendly_err = "PixelDrain upload failed: Network/VPN error - Unable to reach pixeldrain.com."
            else:
                friendly_err = err_str.replace(">", "").replace("<", "")
            
            LOGGER.error(friendly_err)
            await self.listener.on_upload_error(friendly_err)
