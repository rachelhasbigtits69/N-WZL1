    ...
    try:
        ...
        if add_to_queue:
            ...
            if listener.is_cancelled:
                async with jd_listener_lock:
                    stale_ids = (
                        jd_downloads.get(gid, {}).get("ids") if gid else None
                    )
                if stale_ids:
                    try:
                        await jdownloader.device.linkgrabber.remove_links(
                            package_ids=stale_ids
                        )
                    except Exception as e:
                        LOGGER.warning(
                            "JD cancel cleanup: linkgrabber.remove_links "
                            f"failed for gid={gid}: {e}"
                        )
                async with jd_listener_lock:
