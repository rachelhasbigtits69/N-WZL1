...
from bot.helper.ext_utils.files_utils import check_strict_file_mode, get_mime_type
    ...
    def upload(self):
        ...
        try:
            if ospath.isfile(self._path):
                is_allowed, reason = async_to_sync(
                    check_strict_file_mode, self._path, ospath.basename(self._path)
                )
                if not is_allowed:
                    LOGGER.info(f"STRICT_FILE_MODE: Skipping {reason}: {self._path}")
                    async_to_sync(
                        self.listener.on_upload_error,
                        f"Task failed! STRICT_FILE_MODE is enabled. Only videos ≥100MB are allowed. Your file: {reason}"
                    )
                    return
    ...
    def _upload_dir(self, input_directory, dest_id):
        ...
        for item in list_dirs:
            ...
            if ospath.isdir(current_file_name):
                ...
                is_allowed, reason = async_to_sync(
                    check_strict_file_mode, current_file_name, item
                )
                if not is_allowed:
                    LOGGER.info(f"STRICT_FILE_MODE: Skipping {reason}: {current_file_name}")
                    continue
        ...
        if self.total_files == 0 and Config.STRICT_FILE_MODE:
            async_to_sync(
                self.listener.on_upload_error,
                "Task failed! STRICT_FILE_MODE is enabled but no files meet the criteria (videos ≥100MB)."
            )
            return None
            ...
            try:
                ...
                if err.resp.status in [500, 502, 503, 504, 429] and retries < 15:
                    ...
                    if err.resp.status == 429:
                        self._rate_limit_sleep(retries, "upload chunk")
                    continue
