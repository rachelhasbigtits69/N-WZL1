...
class GoogleDriveSearch(GoogleDriveHelper):
    ...
    def _paginated_list(self, list_kwargs):
        """Page through `files().list()` until no `nextPageToken` remains."""
        all_files = []
        page_token = None
        for _ in range(200):
            req_kwargs = dict(list_kwargs)
            if page_token:
                req_kwargs["pageToken"] = page_token
            for attempt in range(1, RATE_LIMIT_MAX_RETRIES + 1):
                try:
                    resp = self.service.files().list(**req_kwargs).execute()
                    break
                except HttpError as err:
                    if self._is_rate_limit_error(err) and attempt < RATE_LIMIT_MAX_RETRIES:
                        self._rate_limit_sleep(attempt, "drive search")
                        continue
                    raise
            ...
            if is_recursive:
                ...
                if dir_id == "root":
                    ...
                    return self._paginated_list({
                        ...
                        "spaces": "drive",
                        "pageSize": 150,
                        "fields": "nextPageToken, files(id, name, mimeType, size, teamDriveId, parents)",
                        "corpora": "drive",
                        "orderBy": "folder, name asc",
                    })
            else:
